...
void send(int _) {
  if (session.currentChatTable != null) {
    ...
    //We insert the new encrypted message into the chattable
    try {
      Statement st = DB.createStatement();
      st.executeUpdate("INSERT INTO " + session.currentChatTable + "(Message, Time, UserID) VALUES "+"('"+messageEncrypted+"', timeofday(),"+ userID +");");
      st.close();
    }
...
void updateChatGroup() {
  ...
  if (session.currentChatTable != null) {
    ...
    messageList.clear();
    //We get all the messages and unencrypt them
    try {
      Statement st = DB.createStatement();
      ResultSet rs = st.executeQuery("SELECT * FROM " + session.currentChatTable + ";");
      ...
      try {
        ...
        while (rs.next()) {
          ...
          if (rs.getInt("UserID") == session.currentUserID) {
    ...
    try {
      Statement st = DB.createStatement();
      ResultSet rs = st.executeQuery("SELECT * FROM CHATS WHERE UserID1 = " + str(session.currentUserID) + " OR UserID2 = " + str(session.currentUserID) + ";");
      //RUNNING THROUGH EACH OF THE CHATS FROM CHATS ADDING IT TO THE LIST OF CHATS
      int i = 0;
      while (rs.next()) {
        String chatname = rs.getString("ChatTableName");
